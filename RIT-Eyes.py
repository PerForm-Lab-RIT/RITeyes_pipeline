...
def seq_render():
    ...
    try:
      os.makedirs(os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2]))
    except OSError:
    ...
    for x in range(1,len(head_x)):
        ...
        camera.location[1]=-4.2
        camera.location[2]=-2.5
        
        camera.rotation_euler[0]=math.radians(122)
        camera.rotation_euler[1]=math.radians(0)
        camera.rotation_euler[2]=math.radians(18)
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],'seq.blend'))
    
    for i in range(1,len(head_x)):      

        s.frame_current = i
        filename=os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],'synthetic' ,str(s.frame_current).zfill(4) + ".tif")
    ...
    for i in range(1,len(head_x)):      
        ...
        filename=os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],'maskwith_skin' ,str(s.frame_current).zfill(4) + ".tif")
        if os.path.isfile(filename):
            print("skipped ",filename)
            continue
        else:
    ...
    for i in range(1,len(head_x)):      
        ...
        frame=i
        s.frame_current = frame
        filename=os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],'maskwithout_skin' ,str(s.frame_current).zfill(4) + ".tif")
        if os.path.isfile(os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],'maskwithout_skin' ,str(s.frame_current).zfill(4) + ".tif")):
            print("skipped ",filename)
            continue
        else:
            print(frame)
    ...
    for x in gt.keys():
        gt[x]=gt[x][1:]
    pickle.dump( gt, open(os.path.join(os.getcwd(),'GIW_Renderings',model,output,p[:-2],model+'-pupil.p'), "wb" ))
    print("saved")
...
def render():
    ...
    for i in range(number):      
        ...
        s.frame_current = i
        filename=os.path.join(os.getcwd(),'static_model',model,output,'synthetic' ,str(s.frame_current).zfill(4) + ".tif")
        if os.path.isfile(filename):
            print("skipped ",filename)
            continue
        else:
    ...
    for i in range(number):      
        ...
        frame=i
        s.frame_current = frame
        filename=os.path.join(os.getcwd(),'static_model',model,output,'maskwith_skin' ,str(s.frame_current).zfill(4) + ".tif")
        if os.path.isfile(filename):
            print("skipped ",filename)
            continue
        else:
    ...
    for i in range(number):      
        ...
        frame=i
        s.frame_current = frame
        filename=os.path.join(os.getcwd(),'static_model',model,output,'maskwithout_skin' ,str(s.frame_current).zfill(4) + ".tif")
        if os.path.isfile(os.path.join(os.getcwd(),'static_model',model,output,'depth' ,str(s.frame_current).zfill(4) + ".tif")):
            print("skipped ",filename)
            continue
        else:
            print(frame)
        ...
        s.render.filepath = os.path.join(os.getcwd(),'static_model',model,output,'depth' ,str(s.frame_current).zfill(4) + ".tif")
