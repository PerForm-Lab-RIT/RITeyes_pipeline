...
parser.add_argument('--pupil_offset_azimuthal', type=int,help='Pupil offset azimuthal',default=0)
parser.add_argument('--pupil_offset_elevation', type=int,help='Pupil offset elevation',default=0)
...
def angle_between_three_3D_points(camera_center, sphere_center, eyeball_points):
    ...
    yaw.append(euler[0])
    return [np.array(yaw) * 180 / np.pi, np.array(roll) * 180 / np.pi, np.array(pitch) * 180 / np.pi ]

 #   return [np.array(yaw), np.array(roll) , np.array(pitch)  ]
...
def blink_render():
   ...
   # print (vec)
    upper_org = angle_between_three_3D_points(cam_loc, [0, 0, 0], [vec[0], vec[1], vec[2]])
    upper_org[0] += math.degrees(upper_blinker.rotation_euler[0])  #AKC: Jackson modified
    
    vec = global_mat @ lower_blinker.tail
  #  print (vec)
    lower_org = angle_between_three_3D_points(cam_loc, [0, 0, 0], [vec[0], vec[1], vec[2]])
    lower_org[0] -= math.degrees(lower_blinker.rotation_euler[0]) #AKC: Jackson modified
    eye_org=angle_between_three_3D_points(cam_loc, [0, 0, 0], [0,0,1])
    eye.keyframe_insert(data_path="rotation_euler", frame=0)
  #  print (upper_org, lower_org,eye_org)
    ...
    ##Upper and lower blinks maximum position based on angles
#    upper_blink_top=[0,-12,-12.8,-6.5,-19.6,-17.6,-10,-12.2,-8.31,-14.8,-9.72,-9.9,-10.3,-8.52,-12.8,-12.8,-15.1,-11.5,-10.7,-11.2,-10.9,-7.63,-15.3,-8.6,-10.6]
#    upper_blink_bottom=[0,40,41.1,40.6,42.4,50.2,50,50.4,42.3,46.5,45.4,44.2,35.4,42.4,44.8,37.3,50.9,39.8,44.6,41.4,42.1,35.9,51.3,33.5,39.3]
#    lower_blink_top=[0,20,13,16.8,12.3,13.7,10.7,6.26,11.1,11.3,9.38,9.5,6.48,7.64,9.42,9.9,5.62,7.66,8.4,9.62,9.42,9.46,10.2,11.6,10.2]
#    lower_blink_bottom=[0,-10.5,-14.7,-16.4,-11.4,-17.3,-14.4,-17.4,-18.4,-8.48,-15.2,-13.1,-15.2,-7.78,-12.5,-14.1,-18,-12.6,-18.4,-13.6,-11.1,-10.5,-10.6,-14.8,-12.7]

#    upper_blink_topvalue=upper_blink_top[int(model)]
#    upper_blink_bottomvalue=upper_blink_bottom[int(model)]
#    lower_blink_bottomvalue=lower_blink_bottom[int(model)]
#    lower_blink_topvalue=lower_blink_top[int(model)]
    ...
    for x in range(1, len(e_x)):
        ...
        #eye.rotation_euler[0] = math.radians(eye_org[0] - e_z[x])
        #eye.rotation_euler[1] = math.radians(eye_org[1] - e_x[x])
        #eye.rotation_euler[2] = math.radians(eye_org[2] + e_y[x])

        # Setting the rotation
        eye.rotation_euler[0] = math.radians(eye_org[0] - e_z[x]+args.pupil_offset_elevation)
        eye.rotation_euler[1] = math.radians(eye_org[1] - e_x[x])
        eye.rotation_euler[2] = math.radians(eye_org[2] + e_y[x]+args.pupil_offset_azimuthal)



     #   upper_blink= np.clip(math.radians(upper_org[0] - u_z[x]),math.radians(upper_blink_topvalue),math.radians(50))
     #   lower_blink= np.clip(math.radians(-lower_org[0]+ l_z[x]),math.radians(lower_blink_bottomvalue),math.radians(20))
#        upper_blink= np.clip(math.radians(upper_org[0]+args.upper_offset - u_z[x]),math.radians(upper_blink_topvalue),math.radians(50)) 
#        lower_blink= np.clip(math.radians(-lower_org[0]+args.lower_offset + l_z[x]),math.radians(lower_blink_bottomvalue),math.radians(20))
        upper_blink= math.radians(upper_org[0] - u_z[x])
        lower_blink= math.radians(-lower_org[0] + l_z[x])
        print ('Blink debug',math.degrees(upper_blink),math.degrees(lower_blink),(upper_org[0] - u_z[x]),lower_org[0] - l_z[x])

        # Setting the rotation

        bpy.data.objects['Armature'].pose.bones['upper_blinker'].rotation_euler[0] = upper_blink#np.clip(math.radians(upper_org[0] - u_z[x]),math.radians(-10),math.radians(51.3))
        bpy.data.objects['Armature'].pose.bones['upper_blinker'].rotation_euler[1] = 0
        bpy.data.objects['Armature'].pose.bones['upper_blinker'].rotation_euler[2] = 0#math.radians(upper_org[2]+ u_y[x])
        bpy.data.objects['Armature'].pose.bones['upper_blinker'].keyframe_insert(data_path="rotation_euler", frame=x)
        bpy.data.objects['Armature'].pose.bones['lower_blinker'].rotation_euler[0] = lower_blink#np.clip(math.radians(lower_org[0] - l_z[x]),math.radians(-10.2),math.radians(10.6)) #-
        bpy.data.objects['Armature'].pose.bones['lower_blinker'].rotation_euler[1] = 0 
        bpy.data.objects['Armature'].pose.bones['lower_blinker'].rotation_euler[2] = 0#math.radians(lower_org[2]+ l_y[x]) #+
        bpy.data.objects['Armature'].pose.bones['lower_blinker'].keyframe_insert(data_path="rotation_euler", frame=x)
